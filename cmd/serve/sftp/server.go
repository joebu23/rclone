...
func (s *server) serve() (err error) {
	...
	// Use the default if not set for authorized keys
	if s.opt.AuthorizedKeys == "" {
		...
		if err == nil {
			...
			if err == nil {
				s.opt.AuthorizedKeys = authKeysPath
			}
		}
	}
	...
	if s.opt.AuthorizedKeys != "" {
		authorizedKeysMap, err = loadAuthorizedKeys(s.opt.AuthorizedKeys)
		if err != nil {
			return errors.Wrap(err, "failed to load authorized keys")
		}
		fs.Logf(nil, "Loaded %d authorized keys from %q", len(authorizedKeysMap), s.opt.AuthorizedKeys)
	}
